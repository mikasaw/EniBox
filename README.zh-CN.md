...
└── tests/
    ├── EniBox.Tests/        # xUnit 测试项目 (177 用例)
...
- **注册表虚拟化为实验性** — 预置读取、子树作用域、写隔离与持久化均有 E2E 覆盖；注意：sidecar 为明文存储（未加密）、删除值/键未挂钩（删除不会从持久化移除条目）、同一封包程序多实例并发为后写赢且无锁（建议单实例运行，删除 sidecar 可重置）、落盘失败仅记录诊断日志不向应用报错
...
| E2E 测试 | 28 | 封包流程、运行时运行、VFS 文件读取、Loader 提取、特殊路径、注册表虚拟化与持久化 |
