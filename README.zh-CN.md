...
- **注册表虚拟化（实验性）** — 封包时可预置注册表键值（`--registry-virtualization` + 编程 API），预置键路径即作用域根，整个子树（含运行时新建键）全部虚拟化；写入不触碰真实注册表，并持久化到封包产物同目录的 `<产物>.vreg.bin`（删除该文件即重置为预置值）；作用域外的键完全透传
...
└── tests/
    ├── EniBox.Tests/        # xUnit 测试项目 (192 用例)
...
- **注册表虚拟化为实验性** — 预置读取、子树作用域、写隔离、持久化与值/键删除均有 E2E 覆盖；注意：sidecar 为明文存储（未加密）、删除作用域根被拒绝（会使子树退出虚拟化）、同一封包程序多实例并发为后写赢且无锁（建议单实例运行）、删除 sidecar 重置为预置值（被删除的预置值会回来）、落盘失败仅记录诊断日志不向应用报错
...
| E2E 测试 | 30 | 封包流程、运行时运行、VFS 文件读取、Loader 提取、特殊路径、注册表虚拟化与持久化 |
