...
class Program
{
    ...
    static int Main(string[] args)
    {
        if (args.Length < 1)
        {
            Console.WriteLine("Usage: FileChecker <file1> [file2] ...");
            return 0;
        }
        ...
        {
            var hFile = CreateFileA(path, GENERIC_READ, FILE_SHARE_READ, IntPtr.Zero, OPEN_EXISTING, 0, IntPtr.Zero);
            if (hFile != INVALID_HANDLE_VALUE)
            {
                ...
                if (ReadFile(hFile, buf, (uint)buf.Length, out uint read, IntPtr.Zero) && read > 0)
                {
                    ...
                    Console.WriteLine($"CHECK:FILE_READ:OK:{path} ({read} bytes, preview: {text.Trim()})");
                }
                else
                {
                    Console.WriteLine($"CHECK:FILE_READ:FAIL:{path} (0 bytes)");
                    allOk = 1;
                }
                CloseHandle(hFile);
            }
            else
            {
                ...
                Console.WriteLine($"CHECK:FILE_OPEN:FAIL:{path} (error={err})");
                allOk = 1;
            }
        }
        return allOk;
